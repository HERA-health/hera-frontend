...
describe('clinic navigation guard', () => {
  ...
  it('keeps team as an active clinic module and future modules disabled', () => {
    ...
    const patientsBlock = source.slice(patientsIndex, source.indexOf("id: 'clinic-agenda'"));
    ...
    expect(patientsBlock).toContain('disabled: true');
  });
});
