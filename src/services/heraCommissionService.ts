...
export interface Entry { id: string; baseCents: number; economicAt: string; recognizedAt: string; reason: string; documentLine: { documentId: string } | null; snapshot: { sessionId: string; position: number | null; rateBps: number | null } }
export interface Document { id: string; rootId: string; originalId: string | null; invoiceNumber: string; storageStatus: 'PENDING' | 'FAILED' | 'READY'; kind: 'ORDINARY' | 'DIFFERENCE' | 'REPLACEMENT'; baseCents: number; taxCents: number; withholdingCents: number; totalCents: number; fileName: string }
export interface DocumentBalance { id: string; invoiceNumber: string; dueAt: string; totalCents: number; appliedCents: number; remainingCents: number; claimableCents: number; correctionPending: boolean; status: string; documents: Document[] }
export interface Cash { id: string; originalId: string | null; kind: 'RECEIPT' | 'REVERSAL' | 'REFUND'; amountCents: number; availableCents: number; occurredAt: string; recordedAt: string; reference: string; applications: Array<{ id: string; documentId: string; amountCents: number; reversalOfId: string | null; reason: string }> }
export interface Issue { id: string; relationId: string | null; snapshotId: string | null; reason: string; status: string; resolution: string | null }
export interface AccountDetail { summary: Balance; acceptances: Acceptance[]; specialistFiscal: { fiscalName: string | null; fiscalNif: string | null; fiscalAddress: string | null }; periods: Period[]; documents: DocumentBalance[]; cash: Cash[]; issues: Issue[]; hasMore: boolean }
export interface Relation { id: string; origin: Origin; status: string; initialCount: number | null }
export interface CommissionSession { id: string; sessionId: string; accountId: string; relationId: string; position: number | null; rateBps: number; bookedGrossCents: number; baseCents: number | null; serviceTaxCents: number | null; potentialCents: number | null; entitledCents: number; exclusion: string | null; revision: number; session: { date: string; clientId: string; attendanceOutcome: string | null; status: string }; relation: Relation; movements: Array<{ id: string; kind: string; grossCents: number; baseCents: number; originalId: string | null; occurredAt: string; reference: string }>; revisions: Array<{ id: string; beforePosition: number | null; position: number | null; beforeBps: number; rateBps: number; deltaCents: number; reason: string }> }
export type Action =
 ...
 | { action: 'RECEIVE'; amountCents: number; occurredAt: string; reference: string; confirmed: true; allowUnapplied: boolean; revision: number; applications: Array<{ documentId: string; amountCents: number }> }
 | { action: 'APPLY_CREDIT'; documentId: string; amountCents: number; revision: number }
...
export const balances = async (filters: { operatorKey: string; mode: Mode; page: number; search?: string; status?: string }) => (await api.get<Page<Balance> & { totals: Pick<Balance, 'pendingCents' | 'accruedCents' | 'undocumentedCents' | 'overdueCents' | 'receivedCents' | 'creditCents'> & { count: number } }>(`${root}/admin/balances`, { params: filters })).data;
