...
export const getReferralBookingQuote = async (id: string, access: ReferralAccess, input: { type: SessionType; duration: number }): Promise<BookingQuote> => (await api.post(`${path(id, access)}/booking/quote`, input, { headers: referralHeaders(access) })).data;
export const bookGuestReferral = async (id: string, access: ReferralAccess, input: { type: SessionType; duration: number; date: string }): Promise<{ id: string; status: SessionStatus }> => (await api.post(`${path(id, access)}/booking/create`, input, { headers: referralHeaders(access) })).data;
