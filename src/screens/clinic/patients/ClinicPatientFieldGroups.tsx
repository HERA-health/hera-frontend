...
import { billingFields, identityFields } from './clinicPatientDomain';
...
interface BillingFieldGroupProps extends FieldGroupProps {
  sameBillingData: boolean;
  onToggleSameBillingData: () => void;
}

export function ClinicPatientIdentityFields({
  form,
  errors,
  disabled,
  onChange,
  ...
  const contactMethodError = errors.email === CONTACT_METHOD_REQUIRED_MESSAGE
    ? errors.email
    : undefined;

  return (
    <View style={styles.fields}>
      {showTitle ? <Text style={styles.groupTitle}>Identidad y contacto</Text> : null}
      ...
      {contactMethodError ? (
        <Text style={[styles.message, { color: theme.error }]}>{contactMethodError}</Text>
      ) : null}
    </View>
  );
}

export function ClinicPatientBillingFields({
  form,
  errors,
  disabled,
  sameBillingData,
  ...
  showTitle = true,
}: BillingFieldGroupProps): React.ReactElement {
  const { theme } = useTheme();
  const styles = useMemo(() => createFormStyles(theme), [theme]);

  return (
    <View style={styles.fields}>
      {showTitle ? <Text style={styles.groupTitle}>Facturación</Text> : null}
      <AnimatedPressable
      ...
      >
        ...
        <View style={styles.billingCopyText}>
          <Text style={styles.billingCopyLabel}>Usar los mismos datos para facturación</Text>
