...
export function Summary({ value }: { value: Pick<Balance, 'pendingCents' | 'accruedCents' | 'undocumentedCents' | 'overdueCents' | 'receivedCents' | 'creditCents'> & Partial<Balance> }) {
  ...
  return <View style={{ gap: 12 }}><View style={[styles.row, { alignItems: 'stretch', gap: 12 }]}>{([
    ['Pendiente documentado', value.pendingCents, 'Comisiones por abonar a HERA', 'time-outline'],
    ['Recibido histórico', value.receivedCents, 'Abonos de comisiones confirmados', 'checkmark-circle-outline'],
    ...
    <CommissionDisclosure title="Ver desglose de saldos" icon="list-outline">
      {([
        ['Devengado sin liquidar', value.accruedCents, 'Comisiones de periodos abiertos.'],
        ['Sin documentar', value.undocumentedCents, 'Periodos cerrados sin importe fiscal definitivo.'],
      ...
      <WorkflowHint>«Pendiente documentado» indica las comisiones que debes abonar a HERA. Las estimaciones y los importes sin documentar aún no son exigibles.</WorkflowHint>
