...
export function Summary({ value }: { value: Pick<Balance, 'pendingCents' | 'accruedCents' | 'undocumentedCents' | 'overdueCents' | 'receivedCents' | 'creditCents'> & Partial<Balance> }) {
  return <View style={[styles.row, { alignItems: 'stretch' }]}>{([
  ...
  ] as const).map(([label, amount, description]) => <Card key={label} style={{ flexGrow: 1, flexBasis: 300 }}><Text>{label}</Text><Text title>{money(amount)}</Text><WorkflowHint>{description}</WorkflowHint></Card>)}</View>;
}
