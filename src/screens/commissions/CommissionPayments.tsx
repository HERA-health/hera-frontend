...
import { Card, Text, Button, WorkflowHint, money, dateTime } from './CommissionElements';
import { CommissionMetrics, LedgerTitle } from './CommissionLedgerUI';

export function CommissionPayments({ data, onDocuments }: { data: AccountDetail; onDocuments: () => void }) {
 const { theme } = useTheme();
 const terms = data.acceptances[0]?.terms;
 const simulation = data.summary.mode === 'SIMULATION';
 return <>
  <Card style={{ padding: 18, gap: 14 }}>
   <LedgerTitle>Pagos confirmados y comisiones pendientes</LedgerTitle>
   <CommissionMetrics items={[
    ...(data.summary.payableCents !== undefined ? [{ label: 'Comisiones y total documentado', value: money(data.summary.payableCents) }] : []),
 ...
 { label: 'Recibido por HERA', value: money(data.summary.receivedCents) },
   ...
   {simulation ? <WorkflowHint>Simulación: estos importes son de prueba. No hagas transferencias reales por esta cuenta.</WorkflowHint> : <>
   ...
   <Button variant="outline" style={{ alignSelf: 'flex-start', maxWidth: '100%' }} onPress={onDocuments}>Ver documentos y descargar facturas</Button>
  </Card>
  <LedgerTitle>Documentos y vencimientos</LedgerTitle>
  {data.documents.length ? data.documents.map(d => <Card key={d.id} style={{ padding: 18, gap: 12 }}>
   <LedgerTitle>{d.invoiceNumber}</LedgerTitle>
   <Text>{d.remainingCents === 0 ? 'Pagado' : d.appliedCents > 0 ? 'Pago parcial' : 'Pago pendiente'} · {d.documents.every(doc => doc.storageStatus === 'READY') ? 'Factura disponible' : 'Factura pendiente'}</Text>
   <Text>Vencimiento: {new Date(d.dueAt).toLocaleDateString('es-ES', { timeZone: 'Europe/Madrid' })}</Text>
   <CommissionMetrics items={[{ label: 'Total', value: money(d.totalCents) }, { label: 'Abonado', value: money(d.appliedCents) }, { label: 'Pendiente de abono', value: money(d.claimableCents), emphasis: true }]} />
   {d.fiscalDifferenceCents ? <Text>Diferencia fiscal respecto a la base de comisión: {money(d.fiscalDifferenceCents)}. El pago previo se conserva.</Text> : null}
   <WorkflowHint>{d.correctionPending ? 'Corrección pendiente. Espera la documentación corregida antes de enviar el pago.' : d.documents.some(doc => doc.storageStatus !== 'READY') ? 'Documento pendiente de completar. Espera a que HERA lo deje disponible.' : d.remainingCents === 0 ? 'Sin importe pendiente en este documento.' : d.appliedCents > 0 ? 'Abono parcial confirmado. Queda el importe pendiente indicado.' : 'Pendiente de abono confirmado por HERA.'}</WorkflowHint>
  </Card>) : <WorkflowHint>No hay documentos en esta página.</WorkflowHint>}
  <LedgerTitle>Abonos y correcciones registrados por HERA</LedgerTitle>
  {!data.cash.length ? <WorkflowHint>No hay movimientos confirmados en esta página. El Bizum o la transferencia que hayas enviado aparecerá cuando HERA registre su recepción.</WorkflowHint> : data.cash.map(c => <Card key={c.id} style={{ padding: 18, gap: 12 }}>
   ...
   <Text>{c.method === 'BIZUM' ? 'Bizum' : c.method === 'BANK_TRANSFER' ? 'Transferencia bancaria' : 'Método no registrado'} · Referencia: {c.reference}</Text>
  ...
  {data.hasMore ? <WorkflowHint>El resumen incluye toda la cuenta. Los documentos y movimientos están paginados; usa «Página siguiente» para consultar el resto.</WorkflowHint> : null}
 </>;
}
