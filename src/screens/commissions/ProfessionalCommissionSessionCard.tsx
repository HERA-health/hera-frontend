...
export function ProfessionalCommissionSessionCard({ row, busy, onActivity, onReview, children }: {
 ...
 return <Card style={{ padding: 16, gap: 10 }}>
  <View style={ledger.header}>
   ...
   <View style={{ gap: 2 }}><NativeText accessibilityLabel={`Comisión generada: ${money(row.entitledCents)}`} style={{ color: theme.primary, fontFamily: theme.fontSansSemiBold, fontSize: 22, lineHeight: 28 }}>{money(row.entitledCents)}</NativeText><WorkflowHint>{row.rateBps / 100}% de {row.baseCents === null ? 'base por confirmar' : money(row.baseCents)}</WorkflowHint></View>
  </View>
  ...
  {details ? <View style={[ledger.details, { borderTopColor: theme.border }]}>
   <WorkflowHint>{origin} · {row.position ? `${row.position}.ª sesión` : 'Orden pendiente de confirmar'}</WorkflowHint>
