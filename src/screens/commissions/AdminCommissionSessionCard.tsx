...
export function AdminCommissionSessionCard({ row, account, run, busy, refresh, onPeriods }: {
 ...
 return <Card style={{ gap: 14, padding: 18 }}>
  ...
  <CommissionMetrics items={[
   ...
   { label: row.position ? `${row.position}.ª sesión` : 'Posición estimada', value: `${row.rateBps / 100}%` },
  ]} />
  ...
  {calculation ? <View style={[ledger.details, { borderTopColor: theme.border }]}>
   ...
   <Text>Comisión potencial: {row.potentialCents === null ? 'Base pendiente de confirmar' : money(row.potentialCents)}. Las confirmaciones de sesiones anteriores pueden modificar la posición y el porcentaje.</Text>
