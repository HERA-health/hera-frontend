...
export function ReferralCheck({ label, checked, onChange, disabled }: { label: string; checked: boolean; onChange: (checked: boolean) => void; disabled?: boolean }) {
  const { theme } = useTheme();
  return <AnimatedPressable accessibilityRole="checkbox" accessibilityLabel={label} accessibilityState={{ checked, disabled: !!disabled }} disabled={disabled} hoverLift={false} pressScale={0.99} onPress={() => onChange(!checked)} style={{ flexDirection: 'row', alignItems: 'center', gap: 12, padding: 14, minHeight: 50, borderWidth: 1, borderRadius: 12, borderColor: checked ? theme.textSecondary : theme.border, backgroundColor: checked ? theme.bg : theme.bgCard, opacity: disabled ? 0.55 : 1 }}><Ionicons name={checked ? 'checkbox' : 'square-outline'} size={22} color={theme.textPrimary} /><Text style={{ flex: 1, color: theme.textPrimary, fontFamily: theme.fontSansMedium, fontSize: 14, lineHeight: 22 }}>{label}</Text></AnimatedPressable>;
}
